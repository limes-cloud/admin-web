    ...
    <ElCard class="art-table-card" shadow="never">
      ...
      <div class="table">
        <ArtTableHeader v-model:columns="columnChecks" :loading="loading" @refresh="refreshData">
          <template #left>
            <ElButton v-permission="'cron:worker_group:add'" v-ripple type="primary" :icon="Plus" @click="showDialog('add')">
              新增分组
            </ElButton>
          </template>
        </ArtTableHeader>
...
<script setup lang="ts">
  ...
  import { ListWorkerGroup, CreateWorkerGroup, UpdateWorkerGroup, DeleteWorkerGroup } from '@/api/cron/worker/api'
  import { WorkerGroup, CreateWorkerGroupRequest, UpdateWorkerGroupRequest } from '@/api/cron/worker/type'
  import { formatTime } from '@/utils/time'

  defineOptions({ name: 'CronWorkerGroup' })

  const dialogType = ref<Form.DialogType>('add')
  const dialogVisible = ref(false)
  const currentData = ref<Partial<WorkerGroup>>({})
  ...
  const operationItems = [
    { icon: Edit, label: '修改', color: 'warning', permission: 'cron:worker_group:update', click: (row: WorkerGroup) => showDialog('edit', row) },
    {
      icon: Delete, label: '删除', color: 'danger', popConfirm: true, permission: 'cron:worker_group:delete',
      click: async (row: WorkerGroup) => { await DeleteWorkerGroup({ id: row.id }); refreshRemove() }
    }
  ]
  ...
  const { columns, columnChecks, data, loading, pagination, searchParams, getData, resetSearchParams, handleSizeChange, handleCurrentChange, refreshData, refreshCreate, refreshUpdate, refreshRemove } =
    useTable({
      core: {
        apiFn: ListWorkerGroup,
        apiParams: { page: 1, pageSize: 10, ...searchForm.value },
        columnsFactory: () => [
          { type: 'index', width: '60', label: '#' },
          { prop: 'name', label: '分组名称' },
          { prop: 'description', label: '分组描述' },
          { prop: 'createdAt', label: '创建时间', formatter: (row: WorkerGroup) => formatTime(row.createdAt) },
  ...
  const showDialog = (type: Form.DialogType, row?: WorkerGroup) => {
  ...
  const handleSubmit = async () => {
    if (dialogType.value === 'add') {
      await CreateWorkerGroup(currentData.value as CreateWorkerGroupRequest)
      ElMessage.success('创建成功'); refreshCreate()
    } else {
      await UpdateWorkerGroup(currentData.value as UpdateWorkerGroupRequest)
