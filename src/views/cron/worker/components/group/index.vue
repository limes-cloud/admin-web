...
import { ListWorkerGroupRequest, WorkerGroup } from '@/api/cron/worker/type';
...
const total = ref(0);
const searchForm = ref<ListWorkerGroupRequest>({
	page: 1,
	pageSize: 10
});
...
const handleGet = async () => {
	setLoading(true);
	try {
		const { data } = await ListWorkerGroup(searchForm.value);
