<template>
	<a-row style="align-items: center; margin-bottom: 16px">
		<a-col :span="12">
			<a-space>
				<a-button v-permission="'cron:task:add'" type="primary" @click="emit('add')">
					<template #icon>
						<icon-plus />
					</template>
					添加任务
				...
				<a-button v-permission="'cron:task:group:query'" type="primary" status="success" @click="emit('showGroup')">
					<template #icon>
						<icon-ordered-list />
					</template>
					任务分组
