<template>
	<a-space direction="vertical" fill>
		<a-table
			v-permission="'notify:channel:query'"
			row-key="id"
			:loading="loading"
			:columns="columns"
			:pagination="false"
		...
		>
			...
			<template #status="{ record }">
				<a-switch v-model="record.status" :disabled="!$hasPermission('notify:channel:update')" type="round" @change="updateStatus(record)">
			...
			<template #operations="{ record }">
				<a-space class="cursor-pointer">
					<a-tag v-permission="'notify:channel:update'" color="orangered" @click="emit('update', record)">
						<template #icon><icon-edit /></template>
						修改
					</a-tag>

					<template v-if="$hasPermission('notify:channel:delete')">
...
import { Channel } from '@/api/notify/channel/type';
import { DeleteChannel, UpdateChannel } from '@/api/notify/channel/api';
