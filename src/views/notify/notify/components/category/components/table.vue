<template>
	<a-space direction="vertical" fill>
		<a-table
			v-permission="'notify:category:query'"
			row-key="id"
			:loading="loading"
			:columns="columns"
			:data="data"
		...
		>
			...
			<template #operations="{ record }">
				<a-space class="cursor-pointer">
					<a-tag v-permission="'notify:category:update'" color="orangered" @click="emit('update', record)">
						<template #icon><icon-edit /></template>
						修改
					</a-tag>

					<a-popconfirm content="您确认删除此分组" type="warning" @ok="handleDelete(record.id)">
						<a-tag v-permission="'notify:category:delete'" color="red">
