<template>
	<div>
		<a-space direction="vertical" fill>
			<a-table
				v-permission="'manager:feedback:query'"
				row-key="id"
				:loading="loading"
				:columns="columns"
				:data="data"
			...
			>
				...
				<template #operations="{ record }">
					<a-space class="cursor-pointer">
						<a-tag v-permission="'manager:feedback:query'" color="arcoblue" @click="handleMore(record)">
							<template #icon><icon-more /></template>
							详情
						</a-tag>

						<a-tag v-permission="'manager:feedback:update'" color="orangered" @click="handleProcessed(record)">
							<template #icon><icon-edit /></template>
							处理
						</a-tag>

						<a-popconfirm content="您确认删除此反馈？" type="warning" @ok="handleDelete(record.id)">
							<a-tag v-permission="'manager:feedback:delete'" color="red">
...
import { Feedback } from '@/api/manager/feedback/type';
import { DeleteFeedback, UpdateFeedback } from '@/api/manager/feedback/api';
