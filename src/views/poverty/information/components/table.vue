<template>
	<a-space direction="vertical" fill>
		<a-table
			v-permission="'poverty:information:query'"
			row-key="id"
			:loading="loading"
			:columns="columns"
			:data="data"
		...
		>
			...
			<template #status="{ record }">
				<a-switch v-model="record.status" :disabled="!$hasPermission('poverty:information:update')" type="round" @change="updateStatus(record)">
			...
			<template #operations="{ record }">
				<a-space class="cursor-pointer">
					<a-tag v-permission="'poverty:information:update'" color="orangered" @click="emit('update', record)">
						<template #icon><icon-edit /></template>
						修改
					</a-tag>

					<a-popconfirm content="您确认删除此资讯？" type="warning" @ok="handleDelete(record.id)">
						<a-tag v-permission="'poverty:information:delete'" color="red">
...
import { DeleteInformation, UpdateInformation } from '@/api/poverty/information/api';
import { Pagination, TableColumn, TableSize } from '@/types/global';
import { Message, Modal } from '@arco-design/web-vue';
import { TableData } from '@arco-design/web-vue/es/table/interface';
import { Information } from '@/api/poverty/information/type';
