<template>
	<a-space direction="vertical" fill>
		<a-table
			v-permission="'poverty:activity:query'"
			row-key="id"
			:loading="loading"
			:columns="columns"
			:data="data"
		...
		>
			...
			<template #status="{ record }">
				<a-switch v-model="record.status" :disabled="!$hasPermission('poverty:activity:update')" type="round" @change="updateStatus(record)">
			...
			<template #operations="{ record }">
				<a-space class="cursor-pointer">
					<a-tag v-permission="'poverty:activity:update'" color="orangered" @click="emit('update', record)">
						<template #icon><icon-edit /></template>
						修改
					</a-tag>

					<a-popconfirm content="您确认删除此活动？" type="warning" @ok="handleDelete(record.id)">
						<a-tag v-permission="'poverty:activity:delete'" color="red">
...
import { DeleteActivity, UpdateActivity } from '@/api/poverty/activity/api';
import { Pagination, TableColumn, TableSize } from '@/types/global';
import { Message, Modal } from '@arco-design/web-vue';
import { TableData } from '@arco-design/web-vue/es/table/interface';
import { Activity } from '@/api/poverty/activity/type';
