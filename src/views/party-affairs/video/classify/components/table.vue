<template>
	<a-space direction="vertical" fill>
		<a-table
			v-permission="'party-affairs:video:classify:query'"
			row-key="id"
			:loading="loading"
			:columns="columns"
			:pagination="false"
		...
		>
			...
			<template #operations="{ record }">
				<a-space class="cursor-pointer">
					<a-tag v-permission="'party-affairs:video:classify:update'" color="orangered" @click="emit('update', record)">
						<template #icon><icon-edit /></template>
						修改
					</a-tag>
					<a-popconfirm content="您确认删除此应用" type="warning" @ok="emit('delete', record.id)">
						<a-tag v-permission="'party-affairs:video:classify:delete'" color="red">
