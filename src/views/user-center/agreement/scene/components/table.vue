<template>
	<a-space direction="vertical" fill>
		<a-table
			v-permission="'uc:agreement:scene:query'"
			row-key="id"
			:loading="loading"
			:columns="columns"
			:pagination="false"
		...
		>
			...
			<template #operations="{ record }">
				<a-space class="cursor-pointer">
					<a-tag v-permission="'uc:agreement:scene:update'" color="orangered" @click="emit('update', record)">
						<template #icon><icon-edit /></template>
						修改
					</a-tag>
					<template v-if="$hasPermission('uc:agreement:scene:delete')">
