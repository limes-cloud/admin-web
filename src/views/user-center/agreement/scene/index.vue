	...
	<div class="container">
		...
		<a-card class="general-card">
			...
			<Table
				...
				@delete="handleDelete"
			></Table>
			<Form ref="formRef" :data="form" :contents="contents" @add="handleAdd" @update="handleUpdate"></Form>
		</a-card>
	</div>
</template>
...
import { pageContent, addScene, deleteScene, pageScene, updateScene } from '@/api/user-center/agreement';
import { Content, PageSceneReq, Scene } from '@/api/user-center/types/agreement';
...
const handleChannel = async () => {
	const { data } = await pageContent({ page: 1, page_size: 50 });
	contents.value = data.list;
};
...
const handleTableUpdate = (data: Scene) => {
	form.value = { ...data };
	const ids = data.contents.map((obj) => obj.id);
	form.value.content_ids = ids;
