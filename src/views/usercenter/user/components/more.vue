<template>
	...
	>
		...
		<a-tabs>
			<a-tab-pane v-if="$hasPermission('uc:auth:query')" key="app">
				<template #title>
					<icon-apps />
					应用列表
				</template>
			...
			<a-tab-pane v-if="$hasPermission('uc:oauth:query')" key="channel">
				<template #title>
					<icon-mind-mapping />
					渠道列表
				</template>
			...
			<a-tab-pane v-if="$hasPermission('uc:userinfo:query')" key="extra">
				<template #title>
					<icon-list />
					详细信息
				</template>
