<template>
	<div class="content">
		<a-button v-permission="'uc:auth:add'" style="margin-bottom: 15px" type="primary" @click="handleClickAddUserApp">
			<template #icon>
				<icon-plus />
			</template>
			新增应用
		...
		>
			...
			<template #status="{ record }">
				<a-space>
					<a-switch v-model="record.status" :disabled="!$hasPermission('uc:auth:update:status')" type="round" @change="updateUserAppStatus(record)">
			...
			<template #operations="{ record }">
				<a-popconfirm content="您确认删除用户的此应用" type="warning" @ok="deleteUserApp(record.appId)">
					<a-tag v-permission="'uc:auth:delete'" color="red" class="cursor-pointer">
...
import { CreateAuth, DeleteAuth, ListAuth, UpdateAuthStatus } from '@/api/usercenter/auth/api';
import { Auth, ListAuthRequest } from '@/api/usercenter/auth/type';
