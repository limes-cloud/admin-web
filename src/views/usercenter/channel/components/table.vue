	...
	<a-space direction="vertical" fill>
		...
		>
			...
			<template #status="{ record }">
				<a-switch v-model="record.status" :disabled="!$hasPermission('uc:channel:update:status')" type="round" @change="updateStatus(record)">
...
import { Channel } from '@/api/usercenter/channel/type';
import { DeleteChannel, UpdateChannelStatus } from '@/api/usercenter/channel/api';
...
const updateStatus = (record: Channel) => {
	...
	Modal.info({
		...
		hideCancel: false,
		onOk: async () => {
			await UpdateChannelStatus({ id: record.id, status: record.status as boolean });
