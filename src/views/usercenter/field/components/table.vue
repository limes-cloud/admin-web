	...
	<a-space direction="vertical" fill>
		...
		>
			<template #status="{ record }">
				<a-switch v-model="record.status" :disabled="!$hasPermission('uc:field:update:status')" type="round" @change="updateStatus(record)">
			...
			<template #operations="{ record }">
				<a-space class="cursor-pointer">
					<a-tag v-permission="'uc:field:update'" color="orangered" @click="emit('update', record)">
						<template #icon><icon-edit /></template>
						修改
					</a-tag>
					<template v-if="$hasPermission('uc:field:delete')">
...
import { Field, FieldType } from '@/api/usercenter/field/type';
import { DeleteField, UpdateFieldStatus } from '@/api/usercenter/field/api';
