<template>
	<a-space direction="vertical" fill>
		<a-table
			v-permission="'application:channel:query'"
			row-key="id"
			:loading="loading"
			:columns="columns"
			:pagination="false"
		...
		>
			...
			<template #status="{ record }">
				<a-switch v-model="record.status" :disabled="!$hasPermission('application:channel:update')" type="round" @change="updateStatus(record)">
			...
			<template #operations="{ record }">
				<a-space class="cursor-pointer">
					<a-tag v-permission="'application:channel:update'" color="orangered" @click="emit('update', record)">
						<template #icon><icon-edit /></template>
						修改
					</a-tag>

					<template v-if="$hasPermission('application:channel:delete')">
...
import { Channel } from '@/api/application/channel/type';
import { DeleteChannel, UpdateChannel } from '@/api/application/channel/api';
