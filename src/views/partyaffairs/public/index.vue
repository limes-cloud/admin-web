...
import { Public, PublicClassify, ListPublicRequest } from '@/api/partyaffairs/public/type';
import { ListPublic, GetPublic, ListPublicClassify } from '@/api/partyaffairs/public/api';
...
const formRef = ref();
const form = ref<Public>({} as Public);
const { setLoading } = useLoading(true);
const loading = ref(false);
const tableData = ref<TableData[]>();
const size = ref<TableSize>('medium');
const total = ref(0);
const classifies = ref<PublicClassify[]>([]);
const showClassify = ref(false);
const searchForm = ref<ListPublicRequest>({
	page: 1,
	pageSize: 10
});
...
const handleGet = async () => {
	setLoading(true);
	try {
		const { data } = await ListPublic(searchForm.value);
...
//  处理tool按钮新建
const handleToolAdd = () => {
	form.value = { isTop: false, status: false } as Public;
	formRef.value.showAddDrawer();
};

// 处理table点击更新
const handleTableUpdate = async (pub: Public) => {
	const { data } = await GetPublic({ id: pub.id });
	form.value = { ...data };
	formRef.value.showUpdateDrawer();
};

// 处理查询
const handleSearch = async (req: ListPublicRequest) => {
