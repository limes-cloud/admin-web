<template>
	<a-space direction="vertical" fill>
		<a-table
			v-permission="'partyaffairs:resource:content:query'"
			row-key="id"
			:loading="loading"
			:columns="columns"
			:data="data"
		...
		>
			...
			<template #operations="{ record }">
				<a-space class="cursor-pointer">
					<a-tag v-permission="'partyaffairs:resource:content:update'" color="orangered" @click="emit('update', record)">
						<template #icon><icon-edit /></template>
						修改
					</a-tag>

					<a-popconfirm content="您确认删除此内容？" type="warning" @ok="emit('delete', record.id)">
						<a-tag v-permission="'partyaffairs:resource:content:delete'" color="red">
