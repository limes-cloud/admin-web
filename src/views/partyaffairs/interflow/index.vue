...
import { InterflowClassify } from '@/api/partyaffairs/interflow/type';

import { ListInterflowClassify } from '@/api/partyaffairs/interflow/api';
...
const handleGet = async () => {
	setLoading(true);
	try {
		const { data } = await ListInterflowClassify();
...
//  处理tool按钮新建
const handleToolAdd = () => {
	form.value = {} as InterflowClassify;
	formRef.value.showAddDrawer();
};

// 处理table点击更新
const handleTableUpdate = async (data: InterflowClassify) => {
	form.value = { ...data };
