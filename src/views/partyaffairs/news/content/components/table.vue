		...
		>
			...
			<template #operations="{ record }">
				<a-space class="cursor-pointer">
					<a-tag v-permission="'partyaffairs:news:comment:query'" color="arcoblue" @click="nav(record.id)">
						<template #icon><icon-message /></template>
						评论
					</a-tag>

					<a-tag v-permission="'partyaffairs:news:content:update'" color="orangered" @click="emit('update', record)">
						<template #icon><icon-edit /></template>
						修改
					</a-tag>

					<a-popconfirm content="您确认删除此内容？" type="warning" @ok="emit('delete', record.id)">
						<a-tag v-permission="'partyaffairs:news:content:delete'" color="red">
