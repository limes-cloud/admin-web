<template>
	<a-row style="align-items: center; margin-bottom: 16px">
		<a-col :span="12">
			<a-space>
				<a-button v-permission="'ai-agent:secret:add'" type="primary" @click="emit('add')">
					<template #icon>
						<icon-plus />
					</template>
					添加密钥
				...
				<a-button v-permission="'ai-agent:secret:group:query'" type="primary" status="success" @click="emit('showGroup')">
					<template #icon>
						<icon-ordered-list />
					</template>
					密钥分组
...
// 定义事件
const emit = defineEmits(['update:size', 'update:columns', 'add', 'showGroup', 'refresh']);
