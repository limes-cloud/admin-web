		...
		>
			...
			<template #operations="{ record }">
				<a-space class="cursor-pointer">
					<a-tag v-permission="'ai-agent:secret:group:update'" color="orangered" @click="emit('update', record)">
						<template #icon><icon-edit /></template>
						修改
					</a-tag>

					<a-popconfirm content="您确认删除此分组" type="warning" @ok="handleDelete(record.id)">
						<a-tag v-permission="'ai-agent:secret:group:delete'" color="red">
...
import { DeleteSecretGroup } from '@/api/ai-agent/secret/api';
